...
router.get("/product_homeApp", (req, res) => {
  var sql = 'SELECT hid, label, picture, title, intro, price, evaluate, source, isFamilyId FROM xm_index_product_homeApp';
  pool.query(sql, (err, result) => {
    if(err) throw err;
    res.send(result);
  })
...
router.get("/product_capacity", (req, res) => {
  var sql = 'SELECT aid, label, picture, title, intro, price, evaluate, source, isFamilyId FROM xm_index_product_capacity';
  pool.query(sql, (err, result) => {
    if(err) throw err;
    res.send(result);
  })
